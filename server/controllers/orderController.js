...
//   const { paymentMode, currencyConversion, ...orderDetails } = req.body;

//   const order = new Order(orderDetails);

//   // We want to only create order when payment is successful. so as its successful we set status to true
//   order.paymentInfo.paymentStatus = true;

//   // saving date of successfull payment
//   order.paymentInfo.paidAt = new Date().toISOString();
...
//       (order?.finalTotalAmountInINR / currencyConversion).toFixed(2)
//     ),
//     currency: order?.currency.toUpperCase(),
//     invoiceProducts: [],
//   };
...
//         (product?.discountPrice / currencyConversion).toFixed(2)
//       )} ${invoiceInfo.currency}`,
//       orderedDate: new Date(order?.paymentInfo?.paidAt).toLocaleString(),
//     });
...
//     console.log(`successfully deleted stock of ${product._id}`);
//   });
...
//   res.status(201).json({
//     message: `Order created successfully! Please check your mail for your order invoice that we sent!`,
//   });
// });
...
// If user entered coupon, then also send this in invoice
// if (order?.couponPrice) {
//   invoiceInfo["couponPrice"] = Number(order?.couponPrice);
// }
